...
export class TilesRenderer extends TilesRendererBase {
	...
	update() {
		...
		while ( cameraInfo.length < cameras.length ) {
			...
			cameraInfo.push( {
				...
				frustum: new Frustum(),
				sseDenominator: - 1,  // Denominator : 分母，共性
		...
		for ( let i = 0, l = cameraInfo.length; i < l; i ++ ) {
			...
			if ( camera.isPerspectiveCamera ) {
				...
				// console.log('info.sseDenominator: ', info.sseDenominator);
				// console.log('camera.fov: ', camera.fov);
				// console.log('resolution.height: ', resolution.height);
				// console.log('Math.tan( 0.5 * camera.fov * DEG2RAD ): ', Math.tan( 0.5 * camera.fov * DEG2RAD ));
	...
	calculateError( tile, base ) {
		...
		const geometricError = base ? base : tile.geometricError;
